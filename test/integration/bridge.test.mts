...
//
// Nothing here is skipped. A missing, stale, or unusable native library fails
// the suite rather than quietly passing it, and an absent licence fails the
// mesh cases on the library's own free-tier error. Both are things the machine
// running these tests is expected to provide.
...
const LIB_NAME = process.platform === "win32" ? "radarsimc.dll"
  : process.platform === "darwin" ? "libradarsimc.dylib"
    : "libradarsimc.so";
const LIB_PATH = path.join(LIB_DIR, LIB_NAME);
...
/**
 * Load the bridge and confirm the library underneath it actually works.
 *
 * Importing binds all ~34 exports -- koffi resolves every lib.func()
 * declaration at import time, so a renamed, removed, or stale export throws
 * here. The probe simulation then confirms the library can really run, which
 * on a machine with no GPU exercises radarsimcpp's runtime CPU fallback.
 *
 * Anything that goes wrong throws out of this hook, failing every test in the
 * file. That is deliberate: an unusable library is a real problem, and a
 * skipped suite looks too much like a passing one.
 *
 * A missing licence is not checked here. The free tier rejects mesh targets,
 * so the mesh cases fail on their own with the library's own message, while
 * the rest still run and mean something.
 */
before(async () => {
  if (!fs.existsSync(LIB_PATH)) {
    throw new Error(
      `native library not found at ${LIB_PATH}. The integration suite needs a ` +
      `radarsimc build for this platform.`);
  }
  ...
  // Smoke-run the simulator so a library that loads but cannot execute fails
  // here, with one clear error, rather than in every individual test.
  await bridge.runSimulation(probeConfig());
});
...
describe("runSimulation", () => {
  test("returns a baseband shaped as [samples, pulses, channels]", async () => {
  ...
  test("baseband nests as [pulse][channel] with matching re/im lengths", async () => {
    const out = await bridge.runSimulation(baseConfig());
    const [spp, pulses, channels] = out.baseband_shape;
  ...
  test("a point target produces a non-zero return", async () => {
    const out = await bridge.runSimulation(baseConfig());
  ...
  test("no targets produces a silent baseband", async () => {
  ...
  test("a more distant target beats at a higher range bin", async () => {
  ...
  test("the beat frequency matches the chirp slope", async () => {
    const range = 30;
    const out = await bridge.runSimulation(baseConfig({
  ...
  test("gate_delay moves the matching target to DC", async () => {
    const range = 30;
  ...
  test("range-doppler output appears only when requested", async () => {
  ...
  test("enabling noise perturbs an otherwise identical run", async () => {
  ...
  test("a real baseband has a zeroed imaginary part", async () => {
  ...
  test("results are JSON-serialisable, as IPC requires", async () => {
    const out = await bridge.runSimulation(baseConfig());
    assert.doesNotThrow(() => JSON.parse(JSON.stringify(out)));
  });

  test("mismatched frequency and time vectors are rejected", async () => {
  ...
  test("a zero-length pulse is rejected rather than silently producing nothing", async () => {
    await assert.rejects(
  ...
  test("a prp shorter than the pulse is rejected", async () => {
    await assert.rejects(
...
describe("getSceneState", () => {
  test("places a stationary radar at its configured location", async () => {
  ...
  test("an unrotated radar looks down +x", async () => {
  ...
  test("a 90 degree yaw in the config turns the boresight to +y", async () => {
  ...
  test("a moving platform advances with the query timestamp", async () => {
  ...
  test("point targets contribute no mesh geometry", async () => {
    const s = await bridge.getSceneState(baseConfig());
    assert.deepEqual(s.meshes, []);
  });

  test("a missing model file is reported as a warning, not a throw", async () => {
    const s = await bridge.getSceneState(baseConfig({
  ...
  test("mesh geometry comes back transformed to the target's pose", async () => {
  ...
  test("mesh rotation is applied, and the config's degrees are converted", async () => {
    const stl = path.join(REPO_ROOT, "test", "fixtures", "plate.stl");
    const s = await bridge.getSceneState(baseConfig({
      targets: [{
  ...
  test("the model unit scales the geometry", async () => {
    const stl = path.join(REPO_ROOT, "test", "fixtures", "plate.stl");
  ...
  test("mesh indices survive an unreadable model earlier in the list", async () => {
    const stl = path.join(REPO_ROOT, "test", "fixtures", "plate.stl");
    const s = await bridge.getSceneState(baseConfig({
      targets: [
